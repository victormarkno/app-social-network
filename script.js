...
function getData() {
  fetch(urlBase)
    ...
    .catch((err) => console.error(`Error al llamar en API: ${err}`));
}
  ...
  fetch(`${urlBase}/${id}`, {
    ...
    .then((data) => {
      ...
      if (index != -1) {
        ...
        alert("Hubo un error al actualizar el posts");
      }

      renderPostsList();
    })
    .catch((error) =>
      console.error(`Hubo un error al actualizar el posts: ${error}`)
    );
}
...
function deletePost(id) {
  ...
  .then(res => {
    if(res.ok){ 
        ...
        alert('Hubo un error al borrar un posts')
    }
  })
  .catch((error) =>
    console.error(`Hubo un error al borrar el posts: ${error}`)
  );
}
